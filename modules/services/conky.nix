...
in
{
  meta.maintainers = [ lib.hm.maintainers.kaleo lib.hm.maintainers.jpoage1 ];
  ...
  config = {
    ...
    home.packages = lib.unique (
      [ cfg.package ]
      ++ (lib.map (conf: conf.package) (
        lib.attrValues (lib.filterAttrs (name: conf: conf.enable) cfg.configs)
      ))
    );
