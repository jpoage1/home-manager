...
in {
  meta.maintainers = [ lib.hm.maintainers.kaleo lib.hm.maintainers.jpoage ];
        ...
        type = lib.types.attrsOf (lib.types.submodule ({ name, ... }: {
          options = {
            ...
            package = lib.mkOption {
              ...
              default = null;
              defaultText =
                lib.literalExpression "config.services.conky.package";
  ...
  config = {
    ...
    systemd.user.services = let
      ...
      manyConky = lib.mapAttrs' (name: conf:
        let
          maybeConf = conf.config;
          isPath = lib.isPath maybeConf;
          configFile = if isPath then
            maybeConf
          else
